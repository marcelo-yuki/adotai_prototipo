...
document.addEventListener('keydown', function (event) {
    if (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'l') {
        ...
        const confirmar = confirm(
            'Deseja apagar todos os dados locais do Adota Aí?\n\n' +
            'Usuários, sessão, perfil, curtidas e interesses serão apagados.'
        );
...
function setImgSourceMode(mode) {
    ...
    modes.forEach(m => {
        ...
        if (m === mode) {
            btn.className = "flex-1 py-1.5 px-2 bg-teal-600 text-white rounded-lg font-bold text-[11px] flex items-center justify-center gap-1";
        } else {
...
function login() {
    ...
    // Mantém email, telefone, senha e role do usuário
    currentUser = user;
...
function register() {
    ...
    if (!email || !phone || !password) {
        alert("Preencha e-mail, telefone e senha para se cadastrar.");
        return;
    }
    ...
    // Telefone passa a fazer parte da conta do adotante
    const newUser = {
        email,
        phone: phoneNumbers,
        password,
...
function logout() {
    ...
    // SEMPRE volta para o formulário de login
    ...
    // Coloca o cursor no e-mail do login
...
function switchAdminTab(tab) {
    ...
    document.getElementById('admin-tab-btn-pets').className = isPets
        ? "flex-1 py-3 text-xs font-bold text-teal-600 border-b-2 border-teal-600 transition-colors flex items-center justify-center gap-1"
    ...
    document.getElementById('admin-tab-btn-interests').className = !isPets
        ? "flex-1 py-3 text-xs font-bold text-teal-600 border-b-2 border-teal-600 transition-colors flex items-center justify-center gap-1 relative"
    ...
    dbDogs.forEach(pet => {
        ...
        card.innerHTML = `
            <div class="relative h-32 w-full bg-gray-100" onclick="openModal(${pet.id})">
                ...
                <div class="absolute top-2 right-2 flex gap-1">
                    <button onclick="event.stopPropagation(); openPetFormModal(${pet.id})" 
                        class="bg-white/90 text-gray-700 hover:text-teal-600 rounded-full w-7 h-7 flex items-center justify-center shadow backdrop-blur-sm transition-colors" title="Editar Pet">
                        <i class="ph-bold ph-pencil-simple text-sm"></i>
                    </button>
                    <button onclick="event.stopPropagation(); deletePet(${pet.id})" 
                        class="bg-white/90 text-red-500 hover:text-red-700 rounded-full w-7 h-7 flex items-center justify-center shadow backdrop-blur-sm transition-colors" title="Excluir Pet">
            ...
            <div class="p-2.5 flex flex-col justify-between flex-1" onclick="openModal(${pet.id})">
                ...
                <p class="text-[10px] text-gray-500 truncate mt-0.5">${pet.breed} • ${pet.age}</p>
            </div>
        `;
...
function renderAdminInterests() {
    ...
    if (adoptionInterests.length === 0) {
        list.innerHTML = `<p class="text-xs text-gray-500 text-center py-8">Nenhum interesse registrado até o momento.</p>`;
    ...
    adoptionInterests.forEach((item, index) => {
        const card = document.createElement('div');
        card.className = "bg-white p-3.5 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-2 text-xs";
        card.innerHTML = `
            <div class="flex items-center justify-between">
                <span class="bg-teal-100 text-teal-800 font-bold px-2 py-0.5 rounded-md text-[10px]">Pet: ${item.petName}</span>
                <div class="flex items-center gap-2">
                    <span class="text-[10px] text-gray-500">${item.date || 'Hoje'}</span>
                    <button onclick="removeInterest(${index})" class="text-red-400 hover:text-red-600 p-1" title="Remover">
                        <i class="ph-bold ph-trash text-base"></i>
                    </button>
                </div>
            </div>
            <div class="space-y-1 text-gray-700">
                <p class="font-bold text-gray-900"><i class="ph-fill ph-envelope mr-1 text-gray-500"></i>${item.userEmail}</p>
                <p><i class="ph-fill ph-phone mr-1 text-gray-500"></i><strong>Tel:</strong> ${item.phone || 'Não informado'}</p>
            </div>
        `;
    ...
    petsToRender.forEach((pet, index) => {
        ...
        card.innerHTML = `
                    <div class="relative w-full h-full bg-cover bg-center" style="background-image: url('${pet.img}')">
                        ...
                        <div class="absolute top-4 left-4 bg-teal-600 text-white font-bold text-xs px-3 py-1.5 rounded-full shadow-md flex items-center gap-1 z-10">
...
function switchTab(tab) {
    ['swipe', 'liked', 'settings'].forEach(t => {
        ...
        if (navBtn) navBtn.className = t === tab
            ? 'flex flex-col items-center justify-center text-teal-600 flex-1 h-full transition-all'
...
function fakeAdoptionAction() {
    if (!currentModalPetId) {
        alert("Não foi possível identificar o pet.");
        return;
    }
    ...
    // Procura os dados completos do usuário cadastrado
    const registeredUser = users.find(
        u => u.email === currentUser.email
    );

    if (!registeredUser) {
        alert("Não foi possível encontrar os dados da sua conta.");
        return;
    }

    const email = registeredUser.email;
    const phone = registeredUser.phone || "Não informado";

    const today = new Date().toLocaleDateString('pt-BR');

    // Evita demonstrar interesse várias vezes no mesmo animal
    const alreadyInterested = adoptionInterests.some(
        interest =>
            interest.petId === pet.id &&
            interest.userEmail === email
    );

    if (alreadyInterested) {
        alert(`Você já demonstrou interesse em adotar ${pet.name}.`);
        return;
    }
    ...
    const newInterest = {
        ...
        userEmail: email,
        phone: phone,
        housing: userProfile?.housing || "Não informado",
        date: today
    };
    ...
    alert(
        ...
        `A ONG poderá entrar em contato pelo telefone cadastrado.`
    );
...
function restoreSession() {
    ...
    // ADOTANTE
    document.getElementById('profile-user-email').innerText =
        currentUser.email;

    if (!userProfile) {
        // Ainda não respondeu o formulário
        document.getElementById('view-profile').classList.remove('hidden');
        document.getElementById('view-profile').classList.add('flex');
    } else {
        // Já possui perfil
